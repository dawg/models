...
def overlap_transform(sample):
    ...
    if trim_frame != 0:
        sample["mix"]["mg"] = np.pad(
            sample["mix"]["mg"],
            ((0, trim_frame), (0, 0)),
            "constant",
            constant_values=(0, 0),
        )
        sample["vocals"]["mg"] = np.pad(
            sample["vocals"]["mg"],
            ((0, trim_frame), (0, 0)),
            "constant",
            constant_values=(0, 0),
    ...
    sample["mix"]["mg"] = stride_tricks.as_strided(
        sample["mix"]["mg"],
        shape=(
            int(sample["mix"]["mg"].shape[0] / (sequence_length - context_length)),
            sequence_length,
            sample["mix"]["mg"].shape[1],
        ),
        strides=(
            sample["mix"]["mg"].strides[0] * (sequence_length - context_length),
            sample["mix"]["mg"].strides[0],
            sample["mix"]["mg"].strides[1],
        ),
    )
    ...
    sample["vocals"]["mg"] = stride_tricks.as_strided(
        sample["vocals"]["mg"],
        shape=(
            int(sample["vocals"]["mg"].shape[0] / (sequence_length - context_length)),
            sequence_length,
            sample["vocals"]["mg"].shape[1],
        ),
        strides=(
            sample["vocals"]["mg"].strides[0] * (sequence_length - context_length),
            sample["vocals"]["mg"].strides[0],
            sample["vocals"]["mg"].strides[1],
        ),
    )
