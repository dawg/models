...
def overlap_transform(sample):
    ...
    sample['mix']['mg'] = overlap_sequences(sample['mix']['mg'], context_length, sequence_length, batch_size)
    sample['vocals']['mg'] = overlap_sequences(sample['vocals']['mg'], context_length, sequence_length, batch_size)
...
def ideal_masking(mixture_in, magn_spectr_target, magn_spectr_residual):
    ...
    """
    mask = np.divide(magn_spectr_target, (_epsilon + magn_spectr_target + magn_spectr_residual))
